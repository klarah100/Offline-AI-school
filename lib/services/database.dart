...
class AppDatabase {
  ...
  Future<Database> get database async {
    ...
    _db=await openDatabase(join(root,'offline_ai_school.db'),version:7,onCreate:(db,_)=>_createSchema(db),onUpgrade:(db,oldVersion,_) async {
      ...
      if(oldVersion<7){await db.execute('ALTER TABLE attempts ADD COLUMN difficulty INTEGER NOT NULL DEFAULT 1');}
    }); return _db!;
  }
  ...
  Future<void> saveAttempt({String? learnerId,required String questionId,required String topicId,required bool correct,String attemptType='practice',String? selectedAnswer,int? durationMs,int difficulty=1,String? clientId}) async {
    final stableLearnerId=learnerId??await this.learnerId(); await (await database).insert('attempts',{'learner_id':stableLearnerId,'client_id':clientId??'attempt_${DateTime.now().microsecondsSinceEpoch}','question_id':questionId,'topic_id':topicId,'correct':correct?1:0,'timestamp':DateTime.now().toIso8601String(),'attempt_type':attemptType,'selected_answer':selectedAnswer,'duration_ms':durationMs,'difficulty':difficulty.clamp(1,3),'synced':0},conflictAlgorithm:ConflictAlgorithm.ignore);
  }
