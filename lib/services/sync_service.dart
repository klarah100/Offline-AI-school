...
class SyncService {
  ...
  static Future<void> _defaultSender(List<Map<String, Object?>> attempts) async {
    ...
    if (baseUrl.isEmpty) {
      throw const SyncException('No sync server configured. Learning data will remain on this device.');
    }
  ...
  Future<int> syncPending({int maxRetries = 3}) async {
    ...
    for (var attempt = 0; attempt < maxRetries; attempt++) {
      try {
        ...
        await database.markAttemptsSynced(pending.map((r) => r['id']).whereType<int>());
